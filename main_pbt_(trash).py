...
if __name__ == "__main__":
    ...
    finish_tasks_q = mp.Queue(maxsize=POPULATION_SIZE + NUM_WORKERS) # Queue that stores finished tasks and worker completion signals.
    ...
    terminated_after_completion = set()
    while True:
        ...
        if round_counter.value >= MAX_PBT_ROUNDS:
            if shutdown_started_at is None:
                shutdown_started_at = time.perf_counter()
            elif time.perf_counter() - shutdown_started_at >= 120:
                for worker in alive_workers:
                    if worker.is_alive():
                        worker.terminate()
                        terminated_after_completion.add(worker.name)
                for worker in alive_workers:
    ...
    failed_processes = [
        ...
        if worker.exitcode not in (0, None) and worker.name not in terminated_after_completion
    ]
    if failed_processes:
        failed_names = ", ".join(f"{worker.name}(exitcode={worker.exitcode})" for worker in failed_processes)
        raise RuntimeError(f"PBT process failed: {failed_names}")
