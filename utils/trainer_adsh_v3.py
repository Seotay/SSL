...
class Trainer:
    def __init__(self, model, labeled_trainloader, unlabeled_trainloader,
        ...
        temperature=1.0, threshold=0.95, momentum_coef=0.8, focal_loss=None,
        use_amp=True,
    ):
        ...
        self.class_conf_stds = torch.full((self.num_classes,), 0.05, dtype=torch.float32, device=self.device)
    ...
    def update_thresholds(self, epoch):
        ...
        for k in range(self.num_classes):
            ...
            if len(Ck) == 0:
                ...
                idx = math.ceil(rho * len(Ck)) - 1
                idx = max(0, min(idx, len(Ck) - 1))
                tau_k = Ck[idx]

            # std_k
            if len(Ck) <= 1:
                std_k = float(self.class_conf_stds[k].detach().cpu().item())
            else:
                std_k = float(torch.std(torch.tensor(Ck, dtype=torch.float32), unbiased=False).item())

            tau_k_list.append(tau_k)
            std_k_list.append(std_k)

        current_thresholds = torch.as_tensor(tau_k_list, dtype=torch.float32, device=self.device).clamp(1e-12, 1.0)
        self.class_conf_stds = torch.as_tensor(std_k_list, dtype=torch.float32, device=self.device).clamp(min=1e-12)
        
        self.class_thresholds = ((1.0 - self.momentum_coef) * current_thresholds + self.momentum_coef * self.class_thresholds).clamp(1e-12, 1.0)
        self.class_thresholds[self.majority_class] = self.threshold

        print(f"[Epoch {epoch + 1}] rho: {rho:.2f}, "
              f"EMA Thresholds: {self.class_thresholds.detach().cpu().numpy().round(2)}, "
              f"Std(C_k): {self.class_conf_stds.detach().cpu().numpy().round(3)}")
        ...
        for step, _ in enumerate(progress_bar, start=1):
            ...
            with torch.amp.autocast("cuda", enabled=self.use_amp):
                ...
                with torch.no_grad():
                    ...
                    std_k = self.class_conf_stds[pseudo_labels]
                    mask = max_probs.ge(tau_k).float() # [B*mu, 1]: True / False
                    reliability_weight = torch.sigmoid((max_probs - tau_k) / (std_k + 1e-12))
           
                loss_u = (mask* reliability_weight * self.focal_loss(logits_u_s, pseudo_labels)).mean()
