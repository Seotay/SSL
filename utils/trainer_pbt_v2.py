...
class Trainer:
    def __init__(self, model, labeled_trainloader, unlabeled_trainloader,
        ...
        temperature=1.0, threshold=0.95, focal_loss=None, mtl_weighting=None,
        use_amp=True,
    ):
    ...
    def train_one_epoch(self, epoch=None):
        ...
        for step, _ in enumerate(progress_bar, start=1):
            ...
            with torch.amp.autocast("cuda", enabled=self.use_amp):
                ...
                loss, alpha, beta = self.mtl_weighting(loss_x, loss_u)
...
class PBT(Trainer):
    ...
    def save_checkpoint(self, checkpoint_path):
        gamma = float(getattr(self.focal_loss, "gamma", 1.5)) if self.focal_loss is not None else 2.0
        checkpoint = {
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        self.mtl_weighting.load_state_dict(ckpt["mtl_state_dict"])
        self.optimizer.load_state_dict(ckpt["optim_state_dict"])
        self.focal_loss.gamma = float(ckpt["gamma"])
    ...
    def training_pbt(self, num_epochs):
        for epoch in range(num_epochs):
            _, _ = self.train_one_epoch(epoch)
